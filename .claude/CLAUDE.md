...
原則として、コードには How、テストコードには What、コミットログには Why、コードコメントには
Why not を書く（[t_wada, 2017](https://x.com/t_wada/status/904916106153828352)）。

- **コード（How）**: どう実現しているか。コメントで説明せず、命名と構造で表す
- **テストコード（What）**: 何が仕様か。テスト名は振る舞いが読み取れる文にする
- **コミットログ（Why）**: 変更の要約に加えて、なぜその変更が必要だったかを書く
- **コードコメント（Why not）**: 一見自然な代替案を採らなかった理由や、コードからは読み取れない
  制約を残す
  - 例:
    - 誤: `// ユーザーを取得する`（コードを読めば分かる How の再説明）
    - 正: `// 並列化すると API のレート制限に引っかかるため直列に回す`

優先順位はまずコードを明瞭にすること。命名や構造で表せない理由だけをコメントにする。
コミットログは変更時点の文脈、コメントは今も有効な制約、と情報の寿命で使い分ける。
...
aws cliを利用するときは下記の注意点を考慮する

- Logs Insightsを利用する際はまずユーザーに判断を仰ぐ
